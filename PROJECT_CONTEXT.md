...
- **The loop start is verified, not assumed** — this is the Safari iOS bug, reported 2026-09-03. `play()` returns a promise that can be **rejected**, and it was being swallowed: the screen lifted onto a frozen frame, permanently. Nothing retried, because `section-loops` watches intersection and intersection had not changed — the loop was already in frame, merely covered by the white screen. Three nets now: `startLoop()` waits for the `playing` event (capped at 900 ms) before lifting; `keepTrying()` retries up to eight times every 450 ms after the lift, for a slow network; and it retries once more **on the first `touchstart`/`pointerdown`**, the only way out under iOS Low Power Mode, which refuses all autoplay, muted included.
- **First-screen images are forced to `eager`** — the other half of the same report: visuals still appearing after the lift. Not slow images, but the carousel cards, injected with `loading="lazy"`; under the boot screen nobody approaches them to trigger the fetch. Whatever falls within the top **1.4 viewports** (14 images max) is switched to `eager` and awaited, with its own 3.8 s cap. Past that, laziness is still right — the rest of the catalogue should not be paid for at open.
- Verified live: forcing `html.booting` / `.boot.done` confirms the white screen renders on both breakpoints; after the fix, `/`, `/tienda`, `/wearables` and `/suplementos` each replay the boot and report `paused:false` with `readyState 4` at reveal (9 images eager, 11 still lazy on the home at 375px).
...
### Card visuals — `card.png`, one per product

Every carousel and grid reads **one** field, `card` (then `packshot`, then `image`), through `cardShot()` in `app.js`. Fixing what a carousel shows is therefore a `catalog.js` edit and nothing else.

The ten `card.png` are generated by `build/card-shots.py` (run by hand — it needs Pillow, which the Node build does not): each source is cropped to its real content box and re-centred in a square with the **same margin for all**, at 88% fill. The packshots come from six different studios and filled anywhere from 55% to 94% of their file, which read as a manufacturing defect across a row of identical tiles. Sources are never touched; the script writes alongside them.

Four visuals were also corrected at the source on 2026-09-03:
- **Venu 4** used `venu-4.png`, the **beige leather** render, while the product page opens on the **Crema** (white) colourway. Now `venu-4-crema.jpg`.
- **Promix Debloat** used `debloat-trio.jpg` — three sticks, all orange. Gallery image 3 (`debloat-sobres.jpg`) shows the three actual flavours.
- **CIRQA** used the Malva cut-out while the product page opens on Negra — the customer saw a mauve band, clicked, and landed on a black one. Now `cirqa-packshot.png`.
- **Promix Non-GMO Creatine** kept the studio's drop shadow in its lower-left corner, which `mix-blend-mode: multiply` turned into a smear on the grey tile. Re-cut from `1-promix-creatine-stick-packs.png` with `build/cutout.swift` (`dist`, 14/26).
- **The replacement band** shows only the blue (`Azul Francés`) in carousels; its seven colours stay on the product page, where they are chosen.
